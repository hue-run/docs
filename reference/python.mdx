...
The [one-command setup workflow](/guides/agent-setup) is the intended interface for repository detection, trial provisioning, Python project edits, real-request verification, and deferred account linkage. Published `@hue-run/sdk@latest` is still 0.3.2 and does not include that 0.4.0 flow. It selects a compatible environment and package manager or returns `action.required`; do not reproduce its credential or private handoff protocol in application code.
