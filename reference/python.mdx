...
| `live_spans` | Default `True`; must be a boolean. Sends a placeholder for each Hue or AI span still open at the export worker's next 0.5 s tick, so Hue shows it as running. Setup keys never send placeholders. See [live spans](#live-spans). |
...
- A span processor that scrubs or rewrites attributes when a span ends, such as a wrapper that forwards an edited copy to Hue's `on_end`, never sees placeholders: the export worker builds each one from the running span's current attributes. `redactor` still applies, because it rewrites helper content before the helper sets it on the span. Scrub any other sensitive attribute where you record it, or pass `live_spans=False`.
...
- A processor wrapper that forwards `on_start` to Hue should also forward `on_end` for the same spans. A placeholder already sent for a span that never reaches Hue can leave the span shown as running.
