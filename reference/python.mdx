...
A placeholder reaches Hue up to about 1.5 s after its span starts: up to 0.5 s until the next tick, then up to 1 s while the export worker batches it. A span shorter than that usually never shows as running and appears only when it ends.
