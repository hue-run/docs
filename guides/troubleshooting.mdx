...
The [Hue MCP server](/agents/mcp-server) authenticates with a project service key sent as `Authorization: Bearer …`. `401` means the key is missing, malformed, revoked or expired. `403` with a message naming **Coding agent (read-only)** means the key is a tracing or evaluation key: create a coding-agent key and configure it as `HUE_MCP_KEY` in the MCP client. `429` follows repeated failed authentications for one key id; wait for `Retry-After`. `503` means the server is momentarily saturated; retry after a second. A `405` or `404` answer to a `GET` or to a request carrying `Mcp-Session-Id` is expected: the server is stateless and accepts `POST` only.
