...
- Sign in with Hue (OAuth) is the MCP path for clients that cannot send a key and for members who cannot create keys. A new sign-in connection acts for the member who approved it, with fixed **Read and write** access to all current and future active projects in one organization. Existing project-scoped approvals retain their project and access until disconnected and approved again. Owner- and admin-only tools check the approving member's current role, and connected apps can never create keys.
