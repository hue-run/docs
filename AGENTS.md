...
- Sign in with Hue (OAuth) is the MCP path for clients that cannot send a key and for members who cannot create keys. A sign-in connection acts for the member who approved it, with the **Read** or **Read and write** access chosen at approval; owner- and admin-only tools check that member's current role, and it can never create keys.
