...
- The agent's `HUE_MCP_KEY` uses **Coding agent (read-only)** and is configured in the MCP client, never in application code. It cannot send or change anything, so a leaked agent key cannot alter your project.
