...
| [Hue MCP server](/agents/mcp-server) | The agent needs live project data: find failing traces, inspect a span, check a receipt, read or author datasets and experiments | Project-scoped tools with pagination, deep links and documentation search. Writes appear only for a **Coding agent (read + evaluations)** key | Key, member or project administration; human-review decisions; OTLP ingest; access to other projects |
...
- The agent's `HUE_MCP_KEY` uses **Coding agent (read-only)** for inspection, or **Coding agent (read + evaluations)** when it should also author datasets, scorers, experiments and environments. Configure it in the MCP client, never in application code.
