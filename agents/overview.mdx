...
| [Hue MCP server](/agents/mcp-server) | The agent needs live project data: find failing traces, inspect a span, check a receipt, read or author evaluations, review work, or inspect settings | Project-scoped tools with pagination, deep links and documentation search. Writes appear only for a **Read and write** key. IAM, consent, taxonomy and managed-target writes need an owner or admin creator | Billing and plan changes; OTLP ingest; artifact bytes; org invites or role changes; access to other projects |
