...
5. `get_trace` on each candidate returns its span tree. Look for spans with `"status": "error"`, and note their `kind`, `name` and `duration_ms`. Open a bounded number, for example the 20 newest, and follow `next_cursor` only if the user asks for more.
...
2. `get_trace` on each trace. Keep the spans with `"kind": "tool"`, and count those with `"status": "error"` by `name`. `get_span` returns the normalized `tool.name` when a span name differs from the tool name.
3. For a failure rate rather than a count, also count the successful tool spans in the same traces. To compare with all traffic, sample traces from `search_traces` without `status` too.
...
### One user's recent traces

Hue has no user filter. The session is the handle:
...
When your application sets a user id, `get_span` returns it as `genAi.userId` on the spans that carry it. See the [field glossary](#field-glossary) for the attributes Hue reads.
...
Model findings need the project's **AI trace findings** setting, which is on by default; see [production safety](/guides/production-safety). The MCP returns the attention state, not the individual findings; the trace's page in Hue lists them.
