...
Connect the server first; [Connect the Hue MCP server](/agents/mcp-server) covers every client. Every tool reads the one project the connection reaches. The [`observe` toolset](/agents/mcp-server#choose-toolsets) lists just the tools on this page, and `load_hue_guide` with topic `production` returns Hue's production guide inside the MCP session.
