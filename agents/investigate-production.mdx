...
Connect the server first; [Connect the Hue MCP server](/agents/mcp-server) covers every client. Call `list_projects` to choose the project to investigate. For an organization connection, pass its `project_id` on every subsequent tool call, including the examples below. Project keys and earlier sign-in connections keep their existing arguments and original project.
...
1. `list_projects` finds the available projects; `get_project_context` confirms the selected project.
...
| Spans per `get_trace` call | `max_spans` from 1 to 500, 200 by default. A page can contain fewer spans to fit `max_chars`; pass `next_span_cursor` back as `span_cursor`. A trace that changes between pages answers `conflict`; start again without the cursor. |
| Result size | `max_chars` defaults to 24,000 and accepts 1,000 to 200,000; values above 80,000 are treated as 80,000. `get_trace` and `get_eval_set` page rows that exceed the budget; `omitted_count` reports rows held for a later page. Shortened content says `truncated: true` with a `hint`. |
...
Trace, span-tree and session list rows omit null and empty fields. When rows share a page URL, the response can provide `<list>_url_template` instead of a link per row; replace `{id}` with the row’s `id`.

A tool that fails returns an MCP error result (`isError: true`) whose text starts with a code, then the message and sometimes a hint, for example `timeout: The query exceeded its time budget. Narrow the window …`. `_meta["hue.run/error"]` carries `code`, `message`, `retryable` and, when present, `hint` and `retry_after_ms`. Invalid arguments identify the bad fields and accepted arguments; use the schema returned by `tools/list`.
