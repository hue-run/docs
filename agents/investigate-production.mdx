...
Connect the server first; [Connect the Hue MCP server](/agents/mcp-server) covers every client. Every tool reads the one project the connection reaches. The [`observe` toolset](/agents/mcp-server#choose-toolsets) lists the tools the recipes below call; add the `intents` group (`observe,intents`) for `get_intent_taxonomy`. `load_hue_guide` with topic `production` returns Hue's production guide inside the MCP session.
