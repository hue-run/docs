...
- **By trace name.** Rows carry `trace_name`, the name your application gives a trace (`hue.trace.name` or `langfuse.trace.name`), and `duration_ms`. Page through `search_traces` over the window and group the rows by `trace_name` to compare durations per task type. For a quick count, `search_traces` with `{"since": "7d", "query": "<name>"}` gives `total_count` for one name, and the same call with `"min_duration_ms": 60000` counts its traces that took a minute or longer; `query` matches the title, the root span name and the trace id, not `trace_name`.
