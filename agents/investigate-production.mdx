...
Connect the server first; [Connect the Hue MCP server](/agents/mcp-server) covers every client. Every tool reads the one project the connection reaches. The [`observe` toolset](/agents/mcp-server#choose-toolsets) lists the tools the recipes below call; `get_intent_taxonomy` is in the `intents` group, which also holds the write tool `save_intent_taxonomy`, so add that group only with a **Read** connection or `?read_only=true`. `load_hue_guide` with topic `production` returns Hue's production guide inside the MCP session.
