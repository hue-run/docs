...
6. `get_span` on a failing span returns `status.message`, the normalized `tool` and `model`, usage and the names of recorded attributes. Add `"include_content": true` to read the tool's arguments and result, or the model's messages. For a large value, `get_span_content` reads just the part you need: see [Read large content](#read-large-content).
...
- **By trace name.** Rows carry `trace_name`, the name your application gives a trace (`hue.trace.name` or `langfuse.trace.name`), and `duration_ms`. Page through `search_traces` over the window and group the rows by `trace_name` to compare durations per task type. `query` does not match `trace_name`: it matches the title, the root span name and the trace id. When the root span is named per task type, `search_traces` with `{"since": "7d", "query": "<root span name>"}` gives a quick `total_count` for it, and the same call with `"min_duration_ms": 60000` counts its traces that took a minute or longer.
...
Each trace row carries `release`: the trace's `service.version` (on the span, else its resource), else `langfuse.release`, else `hue.agent.release`. `search_traces` has no release filter, so page through the window and group rows by `release`, comparing each release's share of `status: "error"` and `attention_state: "needs_attention"`. When the application records no release, compare time windows instead:
...
If several releases serve traffic at once, the windows mix them; group by `release` instead, or say so. Projects set up for trace checks can pass `release` to `get_trace_check_summary`, which matches the request's `hue.agent.release` label.

### One session's or user's recent traces

Each trace row carries `user_id`, the user id your application recorded (see the [field glossary](#field-glossary)). `search_traces` has no user filter, so page through the window and keep the rows whose `user_id` matches. When sessions identify the conversation or user, filter by session instead:
...
A session is not always a user. The result is one user's traces only when the application gives each user a single session; otherwise a user can have several sessions, and a session can be shared. Check each row's `user_id` before attributing traces to a person. An application that derives a sub-agent's user id, for example by appending a call id, gives those traces a different `user_id`. Keep rows whose `user_id` is exactly the base id, such as `user-1`, or starts with the base id and the separator the application appends, such as `user-1:`; never match a bare prefix, which would also match `user-12`. `list_sessions` rows also carry each session's status, trace count and token and span totals over all of its traces.
...
`get_span_content` with `{"trace_id": "<id>", "span_ids": ["<tool span id>"], "fields": ["input", "output"], "length": 1}` returns each value's `total_length` in UTF-16 units while returning one character of it. Pass up to 20 span ids of one trace per call. It is an audited content read like any other, so keep the sample bounded.
...
| `user_id`, `release`, `trace_name` | The user id, release and trace name your application recorded, each up to 200 characters and returned whole. See the [field glossary](#field-glossary). |
...
| Result size | `max_chars` from 1,000, 24,000 by default; any value above 80,000 counts as 80,000, about what clients accept in one result. A result that does not fit is shortened and says `truncated: true` with a `hint`; `get_trace` ends a page early and reports `omitted_count`. |
