...
sidebarTitle: "MCP"
description: "Give Claude Code, Codex, Cursor, Windsurf, Gemini CLI and other MCP clients project-scoped access to traces, evaluations and environments."
---

The Hue project-data MCP server is a hosted [Model Context Protocol](https://modelcontextprotocol.io) server at `https://mcp.hue.run/mcp`. There is nothing to install: point an MCP client at the URL with a coding-agent key. A **Coding agent (read-only)** key can search traces, inspect spans, verify delivery, read datasets, scorers, experiments and environments, and search these docs. A **Coding agent (read + evaluations)** key also lists write tools so the agent can author those objects, all scoped to one project.
...
**Read evaluations.** "List my datasets", "show the cases in the latest version of the regression set", "summarize the last experiment and list the failing cases", "compare it against the baseline experiment". Tools: `list_datasets`, `get_dataset`, `list_scorers`, `get_scorer`, `list_experiments`, `get_experiment`, `list_environments`, `get_environment`.

**Author evaluations.** Requires **Coding agent (read + evaluations)** without `?read_only=true`. Create a dataset (`add_dataset`), add cases (`add_dataset_cases`) or promote a trace (`add_trace_to_dataset`), edit a draft (`update_dataset`, `update_dataset_case`, `delete_dataset_case`, `add_dataset_draft`), freeze it (`freeze_dataset_version`), then pin scorers (`add_scorer`, `publish_scorer`, `copy_scorer`) and create an experiment (`add_experiment`). Case execution stays with the SDK, a managed run, or a local agent. Ask before any write; never auto-approve `add_*`, `update_*`, `delete_*`, `publish_*`, `freeze_*` or `copy_*`.
...
OAuth sign-in for hosted assistants is not in this release. Tool names stay stable; renamed tools keep their old name for at least ninety days. This server does not mint API keys, change members, submit human review, ingest OTLP, or run experiment cases.
