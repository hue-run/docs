...
- `skill.md` mirrors the SDK-owned Hue skill with Mintlify frontmatter adaptations only. `contracts/sources.json` can pin one section or the complete skill to a later SDK commit without changing the released package snapshot. The check binds that override to the base snapshot and exact source digest; a package-snapshot refresh requires renewing or removing it. The current complete-skill override mirrors the skill from a later SDK commit, which adds production investigation over the MCP, independently of package releases.
