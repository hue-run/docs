...
[Install the SDK](/installation) for your language and set `HUE_API_KEY` to a [project service key](/guides/project-keys) created with the **Read and write** preset. A **Tracing only** key cannot use evaluation APIs. Both SDKs connect to Hue Cloud by default. To run an existing TypeScript agent against a disposable simulated world instead of a plain function, see [simulations](/evaluations/simulations).
...
| --- | --- | --- |
| `captureContent` / `capture_content` | `true` | Record the case input and target output in the trace, the recommended setting. |
| `persistResultContent` / `persist_result_content` | `true` | Store the target output so a later run can rescore it. |

Setting the first switch to `false` sends metadata-only telemetry without changing what the evaluation stores.
...
const hue = createHue({ ...connection, serviceName: "local-evaluation", captureContent: true });
try {
...
hue = Hue(api_key=api_key, service_name="local-evaluation", capture_content=True)
try:
    # Run the target locally and save its output for later scoring.
