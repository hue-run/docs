...
[Install the SDK](/installation) for your language and set `HUE_API_KEY` to a [project service key](/guides/project-keys) created with **Read and write** access. A **Read-only** key cannot use evaluation APIs. Both SDKs connect to Hue Cloud by default.
