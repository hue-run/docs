...
[Install the SDK](/installation) for your language and set `HUE_API_KEY` to a [project service key](/guides/project-keys) created with **Tracing and evaluations** access. A **Tracing only** key cannot use evaluation APIs. Both SDKs connect to Hue Cloud by default.
