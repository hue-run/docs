...
```sh
npm install @hue-run/sdk@0.4.2 zod
```

Set `HUE_API_KEY` to a [project service key](/guides/project-keys) created with the **Tracing and evaluations** preset. The evaluation client and the environment client must use the same Hue origin.
...
Running a changed definition while an unfinished simulation is checkpointed in the same directory fails. Recover or finish that simulation first, or use another directory.
