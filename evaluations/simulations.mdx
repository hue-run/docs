...
<Note>
  A bounded metadata-only setup trial does not create or run simulations. First [link the installation to an account](/guides/agent-setup#link-an-account-after-setup), then configure an account-managed **Tracing and evaluations** key for the process that calls these APIs.
</Note>
...
```sh
npm install @hue-run/sdk@0.3.0 zod
```
