...
Use a **Tracing only** project service key from **Settings → Integrations & API keys**. The key determines the destination project; a project ID inside an attribute does not grant access. Use **Tracing and evaluations** only when this process also calls evaluation APIs.
